...
<template>
  ...
  <section class="card focus-card" :class="{ 'compact-list': compactList }" aria-live="polite">
    <div class="card-head"><div><p class="eyebrow">Today’s focus</p><h2>{{ todaysTodos.length }} {{ todaysTodos.length === 1 ? 'task' : 'tasks' }}</h2></div><div class="list-head-actions"><span class="list-readonly">Select a task to begin</span><button v-if="todaysTodos.length" class="density-toggle" :aria-pressed="compactList" @click="toggleCompactList">{{ compactList ? 'Comfortable' : 'Condense' }}</button><button v-if="todaysTodos.length" class="sort-button" aria-label="Sort completed tasks last" title="Sort completed tasks last" @click="sortCompletedLast"><svg viewBox="0 0 24 24" aria-hidden="true"><defs><linearGradient id="completion-sort-gradient" x1="4" y1="3" x2="20" y2="21" gradientUnits="userSpaceOnUse"><stop stop-color="#ef3323"/><stop offset="1" stop-color="#f6bd32"/></linearGradient></defs><path class="sort-ring" d="M8.2 3.1A9.5 9.5 0 1 1 3.1 15.8M3 10.8c.1-1.4.5-2.7 1.2-3.9"/><path class="sort-check" d="m7.2 12.2 3.1 3.1 6.7-7"/></svg></button></div></div>
