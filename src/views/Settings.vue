...
const todos = ref<Todo[]>([]); const title = ref(''); const duration = ref(30); const days = ref<number[]>([1,2,3,4,5]); const scheduleType = ref<'recurring' | 'once'>('recurring'); const error = ref(''); const saving = ref(false)
...
async function add() { if (!title.value.trim() || (scheduleType.value === 'recurring' && !days.value.length) || duration.value < 0) return; saving.value = true; error.value=''; try { const item = await api<Todo>('POST', { title: title.value.trim(), days: scheduleType.value === 'once' ? [] : days.value, one_time: scheduleType.value === 'once', duration_minutes: duration.value }); todos.value.push({ ...item, completed: false }); title.value=''; duration.value=30; days.value=[1,2,3,4,5]; scheduleType.value='recurring'; showAdd.value=false } catch(e) { error.value = e instanceof Error ? e.message : 'Could not save todo' } finally { saving.value=false } }
