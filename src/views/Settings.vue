...
const todos = ref<Todo[]>([]); const title = ref(''); const groupName = ref(''); const duration = ref(30); const days = ref<number[]>([1,2,3,4,5]); const error = ref(''); const saving = ref(false)
...
async function add() { if (!title.value.trim() || !days.value.length || duration.value < 0) return; saving.value = true; error.value=''; try { const item = await api<Todo>('POST', { title: title.value.trim(), group_name: groupName.value.trim() || null, days: days.value, duration_minutes: duration.value }); todos.value.push({ ...item, completed: false }); title.value=''; groupName.value=''; duration.value=30; days.value=[1,2,3,4,5]; showAdd.value=false } catch(e) { error.value = e instanceof Error ? e.message : 'Could not save todo' } finally { saving.value=false } }
