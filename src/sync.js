...
export const syncState = ref('idle');
...
export const syncMessage = computed(() => {
    ...
    if (syncState.value === 'synced' && lastSyncedAt.value)
        return `Synced ${lastSyncedAt.value.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}`;
    return 'Sync';
});
